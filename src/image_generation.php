...

{
    ...
    if ($binary === false || $binary === '') {
        ...
        throw new RuntimeException('生成的图片 base64 解码失败。');
...

{
    ...
    if ($binary === false || $binary === '') {
        ...
        throw new RuntimeException('下载远程媒体失败：' . ($curlError ?: '空响应'));
    ...
    if ($httpCode < 200 || $httpCode >= 400) {
        ...
        throw new RuntimeException('下载远程媒体失败，HTTP ' . $httpCode);
...
function safe_join_api_url(string $baseUrl, string $endpoint): string

{
    ...
    if (preg_match('#/v1$#i', $baseUrl) && preg_match('#^/v1/#i', $endpoint)) {
