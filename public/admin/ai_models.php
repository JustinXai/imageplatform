...
<section class="card" style="margin-bottom:24px;"><div class="card-head"><div><div class="section-badge badge-image">图片模型</div><h2>图片生成 / 编辑模型</h2><p style="font-size:12px;color:var(--text-muted);margin-top:2px;">用于图片绘画、图片编辑、参考图改图</p></div><span class="badge">共 <?= count($imageModels) ?> 个</span></div><?php if (!empty($imageModels)): ?><div style="overflow-x:auto;"><table><thead><tr><th>排序</th><th>名称</th><th>模型 ID</th><th>Base URL</th><th>API Key</th><th>点数/次</th><th>状态</th><th>支持编辑</th><th>编辑接口类型</th><th>最大参考图数</th><th>可选比例</th><th>操作</th></tr></thead><tbody><?php foreach ($imageModels as $m): ?><?php $mid = (int) $m['id']; $isActive = (int) $m['is_active']; $supportsEdit = (int) ($m['supports_edit'] ?? 0); $editAdapter = strtolower(trim((string) ($m['edit_adapter'] ?? 'none'))); $imgAspOpts = decode_json($m['image_aspect_options_json'] ?? null); $imgDefAsp = trim((string) ($m['image_default_aspect'] ?? 'auto')); $imgSzOpts = decode_json($m['image_size_options_json'] ?? null); $imgDefSz = trim((string) ($m['image_default_size'] ?? 'auto')); $maxRefImg = max(1, (int) ($m['max_reference_images'] ?? 1)); ?></tr><?php /* STANDALONE UPDATE FORM */ ?><form method="post" id="upd-img-<?= $mid ?>" style="display:contents;"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="update_model"><input type="hidden" name="id" value="<?= $mid ?>"><input type="hidden" name="image_default_aspect" value="<?= e($imgDefAsp !== '' ? $imgDefAsp : 'auto') ?>"><input type="hidden" name="image_size_options" value="<?= e(implode(',', $imgSzOpts ?: ['auto'])) ?>"><input type="hidden" name="image_default_size" value="<?= e($imgDefSz !== '' ? $imgDefSz : 'auto') ?>"></form><?php /* STANDALONE DELETE FORM */ ?><form method="post" id="del-img-<?= $mid ?>" style="display:contents;"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="delete_model"><input type="hidden" name="id" value="<?= $mid ?>"><input type="hidden" name="confirm_delete" id="cd-img-<?= $mid ?>" value="0"></form><tr><td><input form="upd-img-<?= $mid ?>" name="sort_order" type="text" inputmode="numeric" pattern="[0-9]*" class="compact-input model-num-input model-num-sort" value="<?= (int) $m['sort_order'] ?>"></td><td><input form="upd-img-<?= $mid ?>" name="name" class="compact-input" value="<?= e($m['name']) ?>" required style="min-width:100px;"></td><td><input form="upd-img-<?= $mid ?>" name="model_id" class="compact-input" value="<?= e($m['model_id']) ?>" required style="min-width:110px;"></td><td><input form="upd-img-<?= $mid ?>" name="base_url" class="compact-input" value="<?= e($m['base_url']) ?>" required style="min-width:140px;"></td><td><span class="muted-hint">已配置</span><input form="upd-img-<?= $mid ?>" name="api_key" type="password" class="compact-input" placeholder="留空不修改" autocomplete="off" style="width:80px;"></td><td><input form="upd-img-<?= $mid ?>" name="credits" type="text" inputmode="decimal" pattern="[0-9]+(\.[0-9]+)?" class="compact-input model-num-input model-num-sm" value="<?= e((string) ($m['credits'] ?? '')) ?>" placeholder="点数"></td><td><select form="upd-img-<?= $mid ?>" name="is_active" class="compact-input" style="width:60px;"><option value="1" <?= $isActive===1?'selected':'' ?>>启用</option><option value="0" <?= $isActive!==1?'selected':'' ?>>关闭</option></select></td><td><select form="upd-img-<?= $mid ?>" name="supports_edit" class="compact-input" style="width:72px;"><option value="0" <?= $supportsEdit===0?'selected':'' ?>>否</option><option value="1" <?= $supportsEdit===1?'selected':'' ?>>是</option></select></td><td><select form="upd-img-<?= $mid ?>" name="edit_adapter" class="compact-input" style="min-width:140px;"><option value="none" <?= $editAdapter==='none'?'selected':'' ?>>不支持（默认）</option><option value="newtoken_async_reference" <?= $editAdapter==='newtoken_async_reference'?'selected':'' ?>>newtoken_async_reference</option><option value="nano_banana_image_urls" <?= $editAdapter==='nano_banana_image_urls'?'selected':'' ?>>nano_banana_image_urls</option><option value="openai_edits_multipart" <?= $editAdapter==='openai_edits_multipart'?'selected':'' ?>>openai_edits_multipart</option></select></td><td><input form="upd-img-<?= $mid ?>" name="max_reference_images" type="text" inputmode="numeric" pattern="[0-9]*" class="compact-input model-num-input model-num-xs" value="<?= $maxRefImg ?>"></td><td><input form="upd-img-<?= $mid ?>" name="image_aspect_options" class="compact-input aspect-input" value="<?= e(implode(',', $imgAspOpts)) ?>" placeholder="auto,16:9,9:16"></td><td><div class="table-action-group"><button form="upd-img-<?= $mid ?>" type="submit" class="button secondary small">保存</button><button type="button" class="button danger small" onclick="confirmDeleteModel(<?= $mid ?>, 'img')">删除</button></div></td></tr><?php endforeach; ?></tbody></table></div><?php else: ?><p style="padding:16px;color:var(--text-muted);">暂无图片模型，请在上方添加。</p><?php endif; ?></section>
<section class="card" style="margin-bottom:24px;"><div class="card-head"><div><div class="section-badge badge-video">视频模型</div><h2>视频生成模型</h2><p style="font-size:12px;color:var(--text-muted);margin-top:2px;">用于文生视频、图生视频、首尾帧、多帧参考、视频编辑</p></div><span class="badge">共 <?= count($videoModels) ?> 个</span></div><?php if (!empty($videoModels)): ?><div style="overflow-x:auto;"><table style="min-width:1260px;"><thead><tr><th>排序</th><th>名称</th><th>模型 ID</th><th>Base URL</th><th>API Key</th><th>点/秒</th><th>状态</th><th>视频接口类型</th><th>最大参考图数</th><th>可选时长</th><th>默认时长</th><th>可选比例</th><th>默认比例</th><th>生成模式</th><th>默认模式</th><th>操作</th></tr></thead><tbody><?php foreach ($videoModels as $m): ?><?php $mid = (int) $m['id']; $isActive = (int) $m['is_active']; $vidAdapter = strtolower(trim((string) ($m['video_adapter'] ?? 'none'))); $maxRefImg = max(0, (int) ($m['max_reference_images'] ?? 0)); $durOpts = decode_json($m['video_duration_options_json'] ?? null); $defDur = (int) ($m['video_default_duration'] ?? 0); $aspOpts = decode_json($m['video_aspect_options_json'] ?? null); $defAsp = trim((string) ($m['video_default_aspect'] ?? '16:9')); $szOpts = decode_json($m['video_size_options_json'] ?? null); $defSz = trim((string) ($m['video_default_size'] ?? 'auto')); $modeOpts = decode_json($m['video_mode_options_json'] ?? null); $defMode = trim((string) ($m['video_default_mode'] ?? 'text_to_video')); ?></tr><?php /* STANDALONE UPDATE FORM */ ?><form method="post" id="upd-vid-<?= $mid ?>" style="display:contents;"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="update_model"><input type="hidden" name="id" value="<?= $mid ?>"><input type="hidden" name="video_size_options" value="<?= e(implode(',', $szOpts ?: ['auto'])) ?>"><input type="hidden" name="video_default_size" value="<?= e($defSz !== '' ? $defSz : 'auto') ?>"><input type="hidden" name="video_size_field" value="<?= e(trim((string) ($m['video_size_field'] ?? 'size')) ?: 'size') ?>"></form><?php /* STANDALONE DELETE FORM */ ?><form method="post" id="del-vid-<?= $mid ?>" style="display:contents;"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="delete_model"><input type="hidden" name="id" value="<?= $mid ?>"><input type="hidden" name="confirm_delete" id="cd-vid-<?= $mid ?>" value="0"></form><tr><td><input form="upd-vid-<?= $mid ?>" name="sort_order" type="text" inputmode="numeric" pattern="[0-9]*" class="compact-input model-num-input model-num-sort" value="<?= (int) $m['sort_order'] ?>"></td><td><input form="upd-vid-<?= $mid ?>" name="name" class="compact-input" value="<?= e($m['name']) ?>" required style="min-width:110px;"></td><td><input form="upd-vid-<?= $mid ?>" name="model_id" class="compact-input" value="<?= e($m['model_id']) ?>" required style="min-width:120px;"></td><td><input form="upd-vid-<?= $mid ?>" name="base_url" class="compact-input" value="<?= e($m['base_url']) ?>" required style="min-width:130px;"></td><td><span class="muted-hint">已配置</span><input form="upd-vid-<?= $mid ?>" name="api_key" type="password" class="compact-input" placeholder="留空不修改" autocomplete="off" style="width:80px;"></td><td><input form="upd-vid-<?= $mid ?>" name="credits" type="text" inputmode="decimal" pattern="[0-9]+(\.[0-9]+)?" class="compact-input model-num-input model-num-sm" value="<?= e((string) ($m['credits'] ?? '')) ?>" placeholder="点数"></td><td><select form="upd-vid-<?= $mid ?>" name="is_active" class="compact-input" style="width:60px;"><option value="1" <?= $isActive===1?'selected':'' ?>>启用</option><option value="0" <?= $isActive!==1?'selected':'' ?>>关闭</option></select></td><td><select form="upd-vid-<?= $mid ?>" name="video_adapter" class="compact-input" style="min-width:130px;"><option value="none" <?= $vidAdapter==='none'?'selected':'' ?>>无（默认）</option><option value="newtoken_video_async" <?= $vidAdapter==='newtoken_video_async'?'selected':'' ?>>newtoken（NewToken）</option><option value="kaiyuncode" <?= $vidAdapter==='kaiyuncode'?'selected':'' ?>>kaiyuncode</option></select></td><td><input form="upd-vid-<?= $mid ?>" name="max_reference_images" type="text" inputmode="numeric" pattern="[0-9]*" class="compact-input model-num-input model-num-xs" value="<?= $maxRefImg ?>"></td><td><input form="upd-vid-<?= $mid ?>" name="video_duration_options" class="compact-input duration-input" value="<?= e(implode(',', $durOpts)) ?>" placeholder="8,10,15"></td><td><input form="upd-vid-<?= $mid ?>" name="video_default_duration" type="text" inputmode="numeric" pattern="[0-9]*" class="compact-input model-num-input model-num-xs" value="<?= $defDur ?: '' ?>" placeholder="时长"></td><td><input form="upd-vid-<?= $mid ?>" name="video_aspect_options" class="compact-input aspect-input" value="<?= e(implode(',', $aspOpts)) ?>" placeholder="auto,16:9,9:16"></td><td><input form="upd-vid-<?= $mid ?>" name="video_default_aspect" class="compact-input model-num-xs" value="<?= e($defAsp) ?>"></td><td><div style="display:flex;flex-wrap:wrap;gap:3px;font-size:10px;min-width:200px;"><?php foreach (VIDEO_MODE_OPTIONS as $k => $label): ?><label style="display:flex;align-items:center;gap:2px;cursor:pointer;"><input form="upd-vid-<?= $mid ?>" type="checkbox" class="mode-cb" data-model="<?= $mid ?>" value="<?= e($k) ?>" <?= in_array($k, $modeOpts, true) ? 'checked' : '' ?> onchange="syncModeHidden(<?= $mid ?>)"><?= e($label) ?></label><?php endforeach; ?><input form="upd-vid-<?= $mid ?>" type="hidden" name="video_mode_options" id="modeHidden_<?= $mid ?>" value="<?= e(implode(',', $modeOpts)) ?>"></div></td><td><select form="upd-vid-<?= $mid ?>" name="video_default_mode" class="compact-input" style="width:96px;"><?php foreach (VIDEO_MODE_OPTIONS as $k => $label): ?><option value="<?= e($k) ?>" <?= $defMode===$k?'selected':'' ?>><?= e($label) ?></option><?php endforeach; ?></select></td><td><div class="table-action-group"><button form="upd-vid-<?= $mid ?>" type="submit" class="button secondary small">保存</button><button type="button" class="button danger small" onclick="confirmDeleteModel(<?= $mid ?>, 'vid')">删除</button></div></td></tr><?php endforeach; ?></tbody></table></div><?php else: ?><p style="padding:16px;color:var(--text-muted);">暂无视频模型，请在上方添加。</p><?php endif; ?></section>
<section class="card" style="margin-bottom:24px;"><div class="card-head"><div><div class="section-badge badge-chat">AI 对话模型</div><h2>AI 对话模型</h2><p style="font-size:12px;color:var(--text-muted);margin-top:2px;">用于聊天、文本、工具调用，不参与图片/视频生成</p></div><span class="badge">共 <?= count($chatModels) ?> 个</span></div><?php if (!empty($chatModels)): ?><div style="overflow-x:auto;"><table><thead><tr><th>排序</th><th>名称</th><th>模型 ID</th><th>Base URL</th><th>API Key</th><th>调用方式</th><th>点数</th><th>状态</th><th>操作</th></tr></thead><tbody><?php foreach ($chatModels as $m): ?><?php $mid = (int) $m['id']; $isActive = (int) $m['is_active']; $invokeMode = strtolower(trim((string) ($m['invoke_mode'] ?? 'relay'))); ?></tr><?php /* STANDALONE UPDATE FORM */ ?><form method="post" id="upd-chat-<?= $mid ?>" style="display:contents;"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="update_model"><input type="hidden" name="id" value="<?= $mid ?>"></form><?php /* STANDALONE DELETE FORM */ ?><form method="post" id="del-chat-<?= $mid ?>" style="display:contents;"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="delete_model"><input type="hidden" name="id" value="<?= $mid ?>"><input type="hidden" name="confirm_delete" id="cd-chat-<?= $mid ?>" value="0"></form><tr><td><input form="upd-chat-<?= $mid ?>" name="sort_order" type="text" inputmode="numeric" pattern="[0-9]*" class="compact-input model-num-input model-num-sort" value="<?= (int) $m['sort_order'] ?>"></td><td><input form="upd-chat-<?= $mid ?>" name="name" class="compact-input" value="<?= e($m['name']) ?>" required style="min-width:100px;"></td><td><input form="upd-chat-<?= $mid ?>" name="model_id" class="compact-input" value="<?= e($m['model_id']) ?>" required style="min-width:110px;"></td><td><input form="upd-chat-<?= $mid ?>" name="base_url" class="compact-input" value="<?= e($m['base_url']) ?>" required style="min-width:140px;"></td><td><span class="muted-hint">已配置</span><input form="upd-chat-<?= $mid ?>" name="api_key" type="password" class="compact-input" placeholder="留空不修改" autocomplete="off" style="width:80px;"></td><td><select form="upd-chat-<?= $mid ?>" name="invoke_mode" class="compact-input" style="width:100px;"><option value="relay" <?= $invokeMode==='relay'?'selected':'' ?>>中转站</option><option value="curl" <?= $invokeMode==='curl'?'selected':'' ?>>curl</option></select></td><td><input form="upd-chat-<?= $mid ?>" name="credits" type="text" inputmode="decimal" pattern="[0-9]+(\.[0-9]+)?" class="compact-input model-num-input model-num-sm" value="<?= e((string) ($m['credits'] ?? '')) ?>" placeholder="点数"></td><td><select form="upd-chat-<?= $mid ?>" name="is_active" class="compact-input" style="width:60px;"><option value="1" <?= $isActive===1?'selected':'' ?>>启用</option><option value="0" <?= $isActive!==1?'selected':'' ?>>关闭</option></select></td><td><div class="table-action-group"><button form="upd-chat-<?= $mid ?>" type="submit" class="button secondary small">保存</button><button type="button" class="button danger small" onclick="confirmDeleteModel(<?= $mid ?>, 'chat')">删除</button></div></td></tr><?php endforeach; ?></tbody></table></div><?php else: ?><p style="padding:16px;color:var(--text-muted);">暂无 AI 对话模型，请在上方添加。</p><?php endif; ?></section>
...
(function () {
    ...
    window.confirmDeleteModel = function (id, type) {
        var name = '未知模型';
        var row = document.querySelector('input[form="upd-' + type + '-' + id + '"][name="name"]');
        ...
        var form = document.getElementById('del-' + type + '-' + id);
        if (form) {
            form.submit();
        } else {
